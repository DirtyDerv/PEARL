...
void display_position_info(LCD_I2C& lcd, QuadratureEncoder& encoder) {
    // Clear display and show position information
    lcd.clear();
    ...
    // Line 2: Raw encoder count
    lcd.set_cursor(0, 1);
    lcd.printf("Count: %ld", encoder.get_raw_position());
    
    // Line 3: Calculated distance
    lcd.set_cursor(0, 2);
    lcd.printf("Dist: %.4f units", encoder.get_distance());
...
int main() {
    ...
    printf("Commands: R=reset, S=scan I2C, H=help\n\n");
    ...
    while (true) {
        ...
        if (position_changed || time_to_update) {
            ...
            if (position_changed && ENABLE_SERIAL_DEBUG) {
                printf("Position: %6ld | Distance: %8.4f units | Rate: %.1f Hz\n", 
                       current_position, encoder.get_distance(),
                       1000.0f / (current_time - last_update + 1));
            }
